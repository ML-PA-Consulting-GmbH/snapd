...
func (s *writerSuite) TestCore20NonDangerousDisallowedDevmodeSnaps(c *C) {
	...
	s.makeSnap(c, "my-devmode", constants.GetAccountId())
...
func (s *writerSuite) TestValidateValidationSetsCore20EnforcedHappy(c *C) {
	...
	for _, a := range modelEtc {
		if a.Type() == asserts.ValidationSetType {
			...
			c.Check(vsa.AccountID(), Equals, constants.GetAccountId())
...
func (s *writerSuite) TestValidateValidationSetsCore18EnforcedHappy(c *C) {
	...
	seedAssertsDir := filepath.Join(s.opts.SeedDir, "assertions")
	p := filepath.Join(seedAssertsDir, fmt.Sprintf("16,%s,%s,2.validation-set", constants.GetAccountId(), "base-set"))
	decl := seedtest.ReadAssertions(c, p)
	c.Assert(decl, HasLen, 1)
	c.Check(decl[0].Type(), Equals, asserts.ValidationSetType)
	c.Check(decl[0].HeaderString("account-id"), Equals, constants.GetAccountId())
...
func (s *writerSuite) TestManifestPreProvidedSequenceNotMatchingModelSequence(c *C) {
	...
	s.opts.Manifest.SetAllowedValidationSet(constants.GetAccountId(), "base-set", 1, true)
	...
	err = w.Start(s.db, s.rf)
	c.Assert(err, ErrorMatches, `cannot use sequence 1 of "`+constants.GetAccountId()+`/base-set": model requires sequence 2`)
}
...
func (s *writerSuite) TestManifestPreProvidedSequenceNotMatchingModelPinned(c *C) {
	...
	err = w.Start(s.db, s.rf)
	c.Assert(err, ErrorMatches, `pinning of "`+constants.GetAccountId()+`/base-set" is not allowed by the model`)
}
...
func (s *writerSuite) TestValidateValidationSetsManifestsCorrectly(c *C) {
	...
	// the snaps
	s.MakeAssertedSnap(c, snapYaml["pc-kernel=20"], snapFiles["pc-kernel=20"], snap.R(7), constants.GetAccountId(), s.StoreSigning.Database)
	s.MakeAssertedSnap(c, snapYaml["pc=20"], snapFiles["pc=20"], snap.R(7), constants.GetAccountId(), s.StoreSigning.Database)
	...
	manifest.SetAllowedValidationSet(constants.GetAccountId(), "base-set", 1, false)
...
sequence: 1`)
	...
	c.Assert(err, IsNil)
	c.Check(string(m), Equals, constants.GetAccountId()+`/base-set 1
core20 1
snapd 1
`)
}
