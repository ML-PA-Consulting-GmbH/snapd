...
type systemKey struct {
	...
	// BuildInfoHash is a hash of /etc/buildinfo, when present (eg. on
	// images built with Yocto's image-buildinfo class). It lets images
	// that replace their whole root filesystem on update (eg. via RAUC or
	// similar A/B updaters) signal that security profiles may need
	// regenerating even when the snapd binary's own BuildID is unchanged.
	// It is left empty when the file is absent, which is the case on most
	// distros.
	BuildInfoHash string `json:"buildinfo-hash,omitempty"`
}
...
// buildInfoPath is the optional, distro-provided file carrying image build
// provenance (eg. as generated by Yocto's image-buildinfo class). Its
// presence and contents are entirely optional and distro-specific.
var buildInfoPath = "/etc/buildinfo"

// buildInfoMaxSize bounds how many bytes of buildInfoPath we hash. The system
// key is recomputed on *every* `snap run` and so must stay cheap (see the
// systemKey doc). /etc/buildinfo is a small provenance text file; this cap keeps
// a mis-provisioned or maliciously oversized file from turning system-key
// generation into an expensive read-and-hash.
const buildInfoMaxSize = 1 << 20 // 1 MiB

// buildInfoHash returns a hash of buildInfoPath's contents (up to
// buildInfoMaxSize bytes), or an empty string if the file does not exist on
// this system.
func buildInfoHash() (string, error) {
	path := filepath.Join(dirs.GlobalRootDir, buildInfoPath)

	// Only hash a regular file. /etc/buildinfo is distro-provisioned provenance
	// metadata; if it is missing, or is not a regular file (e.g. a FIFO, device
	// or socket, whose open/read could block indefinitely), treat it as absent.
	// This runs on every `snap run`, so the system key must stay cheap to
	// compute (see the systemKey doc comment) and must never block.
	fi, err := os.Stat(path)
	if err != nil {
	...
	h := sha3.New384()
	if _, err := io.Copy(h, io.LimitReader(f, buildInfoMaxSize)); err != nil {
		return "", err
	}
...
func generateSystemKey() (*systemKey, error) {
	...
	// Add buildinfo-hash, if the optional file is present on this image.
	sk.BuildInfoHash, err = buildInfoHash()
	if err != nil {
		logger.Noticef("cannot determine build info hash in generateSystemKey: %v", err)
		return nil, err
	}
