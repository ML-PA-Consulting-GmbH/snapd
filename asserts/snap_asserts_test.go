...
func (s *baseDeclSuite) TestBuiltinInitErrors(c *C) {
	...
	tests := []struct {
		...
		{"type: base-declaration", `the builtin base-declaration "authority-id" header is not set to expected value "` + constants.AccountId + `"`},
		{"type: base-declaration\nauthority-id: " + constants.AccountId, `the builtin base-declaration "series" header is not set to expected value "16"`},
