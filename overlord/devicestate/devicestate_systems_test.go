...
func (s *deviceMgrSystemsBaseSuite) SetUpTest(c *C) {
	...
	s.model = s.makeModelAssertionInState(c, constants.GetAccountId(), "pc-20", map[string]interface{}{
		"architecture": "amd64",
		// UC20
		"grade": "dangerous",
		"base":  "core20",
...
func (s *deviceMgrSystemsSuite) SetUpTest(c *C) {
	...
	seed20.MakeAssertedSnap(c, "name: pc\nversion: 1\ntype: gadget\nbase: core20", nil, snap.R(1), constants.GetAccountId(), seed20.StoreSigning.Database)
...
func (s *deviceMgrSystemsCreateSuite) makeSnapInState(c *C, name string, rev snap.Revision) *snap.Info {
	...
	// asserted?
	if !rev.Unset() && !rev.Local() {
		s.setupSnapDecl(c, info, constants.GetAccountId())
		s.setupSnapRevision(c, info, constants.GetAccountId(), rev)
	}
...
func (s *deviceMgrSystemsCreateSuite) TestDeviceManagerCreateRecoverySystemRemodelDownloadingSnapsHappy(c *C) {
	...
	s.setupSnapDeclForNameAndID(c, "foo", s.ss.AssertedSnapID("foo"), constants.GetAccountId())
	s.setupSnapRevisionForFileAndID(c, fooSnap, s.ss.AssertedSnapID("foo"), constants.GetAccountId(), snap.R(99))
	snapsupBar := snapstate.SnapSetup{
		SideInfo: &snap.SideInfo{RealName: "bar", SnapID: s.ss.AssertedSnapID("bar"), Revision: snap.R(100)},
		SnapPath: barSnap,
	}
	s.setupSnapDeclForNameAndID(c, "bar", s.ss.AssertedSnapID("bar"), constants.GetAccountId())
	s.setupSnapRevisionForFileAndID(c, barSnap, s.ss.AssertedSnapID("bar"), constants.GetAccountId(), snap.R(100))
	...
	newModel := s.brands.Model(constants.GetAccountId(), "pc-20", map[string]interface{}{
		"architecture": "amd64",
		// UC20
		"grade": "dangerous",
		"base":  "core20",
