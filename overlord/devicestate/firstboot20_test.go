...
`
	...
	makeSnap := func(yamlKey string) {
		...
		s.MakeAssertedSnap(c, s.snapYaml(yamlKey), files, snap.R(1), constants.GetAccountId(), s.StoreSigning.Database)
	}
...
func (s *firstBoot20Suite) TestPopulateFromSeedCore20ValidationSetTrackingHappy(c *C) {
	...
	chg := s.testPopulateFromSeedCore20ValidationSetTracking(c, []string{constants.GetAccountId() + "/base-set/1"})
	...
	err = assertstate.GetValidationSet(s.overlord.State(), constants.GetAccountId(), "base-set", &tr)
	c.Assert(err, IsNil)
	c.Check(tr, DeepEquals, assertstate.ValidationSetTracking{
		AccountID: constants.GetAccountId(),
		Name:      "base-set",
...
func (s *firstBoot20Suite) TestPopulateFromSeedCore20ValidationSetTrackingFailsUnmetCriterias(c *C) {
	...
	chg := s.testPopulateFromSeedCore20ValidationSetTracking(c, []string{constants.GetAccountId() + "/base-set/2"})

	s.overlord.State().Lock()
	defer s.overlord.State().Unlock()
	c.Assert(chg.Status(), Equals, state.ErrorStatus)
	c.Check(chg.Err().Error(), testutil.Contains, "my-snap (required at revision 1 by sets "+constants.GetAccountId()+"/base-set))")
}
