...
func (s *deviceMgrBootconfigSuite) setupUC20ModelWithGrade(c *C, grade string) *asserts.Model {
	...
	headers["grade"] = grade
	return s.makeModelAssertionInState(c, constants.GetAccountId(), "pc-model-20", headers)
}
...
func (s *deviceMgrBootconfigSuite) setupClassicWithModesModel(c *C) *asserts.Model {
	...
	return s.makeModelAssertionInState(c, constants.GetAccountId(), "classic-with-modes",
